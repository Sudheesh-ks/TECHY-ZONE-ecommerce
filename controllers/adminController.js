...
const loadDashboard = async (req, res) => {
  try {
    ...
    const productLabels = topProductsData.map(
      (data) => data.productName || "Unknown Product",
    );
    const productValues = topProductsData.map((data) => data.totalSold);

    const categoryLabels = topCategoriesData.map(
      (data) => data.categoryName || "Unknown Category",
    );
...
const loadSalesData = async (req, res) => {
  try {
    ...
    const orders = await Order.find(filterCondition)
      ...
      .select(
        "userId products paymentMethod totalPrice discountAmount createdAt",
      );
  ...
  try {
    ...
    if (orders.length === 0) {
      return res
        ...
        .json({ success: false, message: "No data available to export." });
    }
    ...
    res
      ...
      .json({
        success: false,
        message:
          error.message || "An error occurred while exporting the sales data.",
      });
  }
};
...
const exportSalesToExcel = async (req, res) => {
  try {
    ...
    if (orders.length === 0) {
      return res
        .status(STATUS_CODES.NOT_FOUND)
        .json("No data available to export.");
    }
